...
import DonorSidebar from './DonorSidebar.jsx'
...
export default function DonorLayout({ children }) {
  ...
  const weeklyFamilies = useMemo(() => {
    ...
    const mine = donations.filter((d) => isOwnDonation(d, user) && d.status === 'delivered' && new Date(d.submittedAt).getTime() >= weekAgo)
    return mine.length
  }, [donations, user])
